...
#include "../../../sim/src/common/commonHelper.h"
                                                    ...
                                                    double range) const
{
    ...
    { // check if viewing direction is infinity --> no specific viewing direction
        nearestPoint = CalcNearestPointFromPoint(point);
             ...
             || (std::isinf(range)))
    { //Check if range is INFINITY --> no range, just direct viewing line
        nearestPoint = CalcIntersectionPointInDirection(point, viewAngle);
    }
    else if ((2 * M_PI - range) <
             std::numeric_limits<double>::lowest())
    { // check if range > 2PI -->no specific viewing direction
        nearestPoint = CalcNearestPointFromPoint(point);
    }
    else
    { // calculate min distance in range about the viewing direction
                                                             ...
                                                             double viewAngle) const
{
    ...
    {
        if ((qFuzzyIsNull(firstPointVec.y)) || (qFuzzyIsNull(secondPointVec.y)))
        { // Check if any point is on x-axis
            // Check if both points are on x-axis
            if (qFuzzyCompare(firstPointVec.y + 1, secondPointVec.y + 1))
            { // +1 for comparing, 0==0 will fail
                if ((firstPointVec.x > 0) && (secondPointVec.x > 0))
                {
...
PCM_Point PCM_LineSegment::CalcNearestPointFromPoint(const PCM_Point *point) const
{
    ...
    PCM_Point nearestPoint;
    if (pointVec.x < 0)
    { // point is left outside lineSegVector range
        nearestPoint = firstPoint;
    }
    else if (pointVec.x > lineSegVector.x)
    { //point is right outside lineSegVector range
                                                                ...
                                                                double viewAngle, double range) const
{
    ...
    if (firstVecIsInRange && secondVecIsInRange)
    {
        ...
        { // whole line segment is in range --> calculate as common distance from point
            transNearestPoint = transLineSeg.CalcNearestPointFromPoint(&origin);
        }
    }
    else
    { // one or none point is in range --> line segment intersects range
