...
EnvironmentPresenter::EnvironmentPresenter(EnvironmentInterface * const environment,
    ...
    , undoStack(undoStack)
    , timeOfDayPresenter( new EnvironmentMapPresenter(environment,
                                                      environment->getTimeOfDayMap(),
                                                      undoStack,
                                                      this))
    , visibilityDistancePresenter( new EnvironmentMapPresenter(environment,
                                                               environment->getVisibilityDistanceMap(),
                                                               undoStack,
                                                               this))
    , frictionPresenter( new EnvironmentMapPresenter(environment,
                                                     environment->getFrictionMap(),
                                                     undoStack,
                                                     this))
    , weatherPresenter(new EnvironmentMapPresenter( environment,
                                                    environment->getWeatherMap(),
                                                    undoStack,
                                                    this))
{
    SetupTimeOfDay();
