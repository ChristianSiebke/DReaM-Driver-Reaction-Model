...
VehicleProfileItemModel::VehicleProfileItemModel(QObject *parent)
    : VehicleProfileItemInterface(parent)
    , modelCatalogue("")
    , model("")
    , systemConfig("")
    , systemProfiles(new SystemProfiles)
    , components(new VehicleComponentMapModel(this))
    , sensors( new SensorMapModel(this))
{
}

VehicleProfileItemModel::VehicleProfileItemModel(VehicleProfileItemInterface const * const vehicle, QObject *parent)
    : VehicleProfileItemInterface(parent)
    , modelCatalogue(vehicle->getModelCatalogue())
    , model(vehicle->getModel())
    , systemConfig(vehicle->getSystemConfig())
    , systemProfiles(new SystemProfiles)
    , components(new VehicleComponentMapModel(this))
    , sensors( new SensorMapModel(this))
{
