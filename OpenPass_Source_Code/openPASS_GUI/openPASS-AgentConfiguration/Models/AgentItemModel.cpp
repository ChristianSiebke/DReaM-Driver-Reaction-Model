...
AgentItemModel::AgentItemModel(QObject *parent)
    : AgentItemInterface(parent)
    , vehicleProfiles(new VehicleProfiles)
    , driverProfiles(new DriverProfiles)
    , type("Dynamic")
    , systemConfig("")
    , system( System() )
    , vehicleModelCatalogue("")
{
}
...
AgentItemModel::AgentItemModel(AgentItemInterface const * const agent, QObject *parent)
    ...
    , driverProfiles(new DriverProfiles)
    , type(agent->getType())
    , systemConfig(agent->getSystemConfig())
    , system(System(agent->getSystemID(), agent->getSystemTitle()))
    , vehicleModelCatalogue(agent->getVehicleModelCatalogue())
    , vehicleModel(agent->getVehicleModel())
{
