...
AgentConfigurationCombinationPresenter::AgentConfigurationCombinationPresenter(AgentConfigurationInterface *agentConfiguration,
    ...
    , vehicleProfiles(agentConfiguration->getVehicleProfiles())
    , sensorProfiles(agentConfiguration->getSensorProfiles())
    , componentProfiles(agentConfiguration->getVehicleComponentProfiles())
    , system(system)
    , project(project)
{
...
void AgentConfigurationCombinationPresenter::addSystem(const QString &systemTitle)
{
  ...
  //  type = currentAlgo->getLibrary();
    ...
    // current system!
    if(!currentVehicleProfile->getVehicleComponents()->getItem(type)
            ->getProfiles()->contains(componentProfiles->getItem(componentProfile_id)))
    {
        double probability = currentVehicleProfile->getSystems(type)->value(QPair<unsigned int, QString>
                                                                      (currentSystem->getID(), currentSystem->getTitle()));

        currentVehicleProfile->getVehicleComponents()->getItem(type)
            ->getProfiles()->insert(componentProfiles->getItem(componentProfile_id), probability);
