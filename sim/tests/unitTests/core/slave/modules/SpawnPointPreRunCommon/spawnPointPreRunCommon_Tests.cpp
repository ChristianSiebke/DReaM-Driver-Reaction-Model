...
TEST(SpawnPointPreRunCommonParameterExtractor, GetWayToNextRoad_LinearGraph)
{
    RoadGraph roadGraph;
...
TEST(SpawnPointPreRunCommonParameterExtractor, GetWayToNextRoad_BranchingGraph)
{
    RoadGraph roadGraph;
    auto start = add_vertex(RouteElement{"RoadStart", true}, roadGraph);
...
TEST(SpawnPointPreRunCommonParameterExtractor, GetWayToNextRoad_NoPossibleWay)
{
    RoadGraph roadGraph;
    auto start = add_vertex(RouteElement{"RoadStart", true}, roadGraph);
    auto node1a = add_vertex(RouteElement{"Road1a", false}, roadGraph);
...
TEST(SpawnPointPreRunCommonParameterExtractor, ExtractSpawnAreas_AllOptionalParameters)
{
    ...
    std::map<std::string, ParameterInterface::ParameterLists> parameterLists{{"SpawnPoints", spawnPoints}};
...
TEST(SpawnPointPreRunCommonParameterExtractor, ExtractSpawnAreas_NoOptionalParameters)
{
    FakeCallback callbacks;
...
TEST(SpawnPointPreRunCommonParameterExtractor, ExtractSpawnAreas_AgaintOdDirection)
{
    FakeCallback callbacks;
    ...
    ON_CALL(fakeWorld, GetRoadGraph(RouteElement{"RoadA", false}, _)).WillByDefault(Return(std::make_pair(roadGraph, nodeA)));
...
TEST(SpawnPointPreRunCommonParameterExtractor, ExtractSpawnAreas_AllLanesBothDirection)
{
    FakeCallback callbacks;
    ...
    ON_CALL(fakeWorld, GetRoadGraph(RouteElement{"RoadA", true}, _)).WillByDefault(Return(std::make_pair(roadGraphAIn, node0)));
    RoadGraph roadGraphAAgainst;
    auto node2 = add_vertex(RouteElement{"RoadA", false}, roadGraphAAgainst);
    ON_CALL(fakeWorld, GetRoadGraph(RouteElement{"RoadA", false}, _)).WillByDefault(Return(std::make_pair(roadGraphAAgainst, node2)));
    ...
    ON_CALL(fakeWorld, GetRoadGraph(RouteElement{"RoadB", false}, _)).WillByDefault(Return(std::make_pair(roadGraphBAgainst, node3)));
