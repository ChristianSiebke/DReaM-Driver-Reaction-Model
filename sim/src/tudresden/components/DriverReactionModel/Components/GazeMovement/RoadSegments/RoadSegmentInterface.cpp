...
double RoadSegmentInterface::UpdateUFOVAngle(GazeState currentGazeState) {
    ...
    case GazeType::ScanGlance:
        if (currentGazeState.fixationState.second == static_cast<int>(ScanAOI::ShoulderCheckLeft)) {
            directionUFOV = ShoulderCheckLeft(ScanAOI::ShoulderCheckLeft).directionUFOV;
        }
        else if (currentGazeState.fixationState.second == static_cast<int>(ScanAOI::ShoulderCheckRight)) {
            directionUFOV = ShoulderCheckRight(ScanAOI::ShoulderCheckRight).directionUFOV;
        }
        else {
            directionUFOV = ScanUFOVAngle(static_cast<ScanAOI>(currentGazeState.fixationState.second));
        }
        break;
    case GazeType::ObserveGlance:
        if (auto oAgent = Common::FindAgentById(currentGazeState.target.fixationAgent, *worldRepresentation.agentMemory)) {
            directionUFOV = CalculateGlobalViewingAngle(oAgent->GetRefPosition() - worldRepresentation.egoAgent->GetGlobalDriverPosition());
        } else {
            directionUFOV = currentGazeState.directionUFOV;
        }
        break;
    case GazeType::ControlGlance:
        directionUFOV = CalculateGlobalViewingAngle(currentGazeState.target.fixationPoint - currentGazeState.startPosUFOV);
        break;
    default:
        std::string message = __FILE__ " Line: " + std::to_string(__LINE__) + " UFOVAngle cannot be updated";
        throw std::runtime_error(message);
    }
    return directionUFOV;
}

Common::Vector2d RoadSegmentInterface::UpdateStartPosUFOV(GazeState currentGazeState) {
    Common::Vector2d pos = worldRepresentation.egoAgent->GetGlobalDriverPosition();
    auto aoi = static_cast<ScanAOI>(currentGazeState.fixationState.second);

    if (behaviourData.gmBehaviour.scanAOIs.mirrorAOIs.find(aoi) != behaviourData.gmBehaviour.scanAOIs.mirrorAOIs.end()) {
        auto relMirrorPos = behaviourData.gmBehaviour.scanAOIs.mirrorAOIs.at(aoi).pos;
        relMirrorPos.Rotate(worldRepresentation.egoAgent->GetYawAngle());
        pos.Add(relMirrorPos);
    }
    return pos;
}
double RoadSegmentInterface::ScanUFOVAngle(ScanAOI aoi) {
    ...
    if (behaviourData.gmBehaviour.scanAOIs.driverAOIs.find(aoi) != behaviourData.gmBehaviour.scanAOIs.driverAOIs.end()) {
        directionUFOV = behaviourData.gmBehaviour.scanAOIs.driverAOIs.at(aoi).direction * (M_PI / 180);
    }
    else if (behaviourData.gmBehaviour.scanAOIs.mirrorAOIs.find(aoi) != behaviourData.gmBehaviour.scanAOIs.mirrorAOIs.end()) {
        directionUFOV = behaviourData.gmBehaviour.scanAOIs.mirrorAOIs.at(aoi).direction * (M_PI / 180);
        mirrorGaze = true;
    }
    else {
        directionUFOV = 0;
...
GazeState RoadSegmentInterface::ScanGlance(CrossingPhase phase) {
    ...
    if (behaviourData.gmBehaviour.scanAOIs.driverAOIs.find(aoi) != behaviourData.gmBehaviour.scanAOIs.driverAOIs.end()) {
        ...
        auto relMirrorPos = behaviourData.gmBehaviour.scanAOIs.mirrorAOIs.at(aoi).pos;
        relMirrorPos.Rotate(worldRepresentation.egoAgent->GetYawAngle());
        auto driverPos = worldRepresentation.egoAgent->GetGlobalDriverPosition();
        driverPos.Add(relMirrorPos);
        gazeState.startPosUFOV = driverPos;
    }
    else {
        std::string message =
...
GazeState RoadSegmentInterface::ShoulderCheckRight(ScanAOI gaze) {
    ...
    gazeState.directionUFOV = worldRepresentation.egoAgent->GetYawAngle() - 110 * (M_PI / 180);
    gazeState.openingAngle = behaviourData.gmBehaviour.observe_openingAngle;
    gazeState.viewDistance = 100;
    DReaM::NormalDistribution *de = behaviourData.gmBehaviour.observe_fixationDuration.get();
    double dist = stochastics->GetNormalDistributed(de->mean, de->std_deviation);
...
GazeState RoadSegmentInterface::MirrowGaze(ScanAOI aoi) {
    ...
    Junction::Junction(const WorldRepresentation &worldRepresentation, StochasticsInterface *stochastics,
    ...
    Junction::OncomingStraightConnectionLane(const MentalInfrastructure::Junction *currentJunction) const {
