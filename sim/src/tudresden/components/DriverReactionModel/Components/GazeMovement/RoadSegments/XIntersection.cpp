...
XIntersection::XIntersection(const WorldRepresentation& worldRepresentation, StochasticsInterface* stochastics, const BehaviourData& behaviourData)
    : Intersection(worldRepresentation, stochastics, behaviourData) {
    ...
    CalculateControlFixPointsOnXintersection();
    CalculateControlFixPointsOnRoads();

    controlFixationPoints.insert(controlFixationPoints.begin(), controlFixPointsOnXIntersection.begin(),
                                 controlFixPointsOnXIntersection.end());
    controlFixationPoints.insert(controlFixationPoints.end(), controlFixPointsOnRoads.begin(), controlFixPointsOnRoads.end());
}

void XIntersection::CalculateControlFixPointsOnXintersection() {
    auto nextIntersection = worldRepresentation.egoAgent->NextIntersection();
    auto cornerSidewalkLanes = CornerSidewalkLanesOfIntersection(nextIntersection);
    ...
    for (auto sidewalkLane : cornerSidewalkLanes) {
        ...
        controlFixPointsOnXIntersection.push_back({point.x, point.y});
    }

    auto straightConLane = OncomingStraightConnectionLane(nextIntersection);
    ...
    controlFixPointsOnXIntersection.push_back(oncomingPoint);

    // sketch of sorted intersection fixation points
    ...
    //         | |A |
    //
    SortControlFixPoints(controlFixPointsOnXIntersection);

    if (controlFixPointsOnXIntersection.size() != 5) {
        std::string message =
            __FILE__ " Line: " + std::to_string(__LINE__) + "the number of control fixation points on X-Intersection is incorrect";
        throw std::runtime_error(message);
    }
};

void XIntersection::CalculateControlFixPointsOnRoads() {
    auto NextIntersection = worldRepresentation.egoAgent->NextIntersection();
    const auto& IncomingIntersectionRoadIds = NextIntersection->GetIncomingRoads();
    for (auto incomingRoad : IncomingIntersectionRoadIds) {
        if (worldRepresentation.egoAgent->GetRoad() == incomingRoad) {
            continue;
        }
        auto conRoads = NextIntersection->GetConnectionRoads(incomingRoad);
    ...
    if (controlFixPointsOnRoads.size() != 3) {
        std::string message = __FILE__ " Line: " + std::to_string(__LINE__) +
                              "the number of control fixation points on incoming roads (X-Intersection) is incorrect";
        throw std::runtime_error(message);
    }
}

GazeState XIntersection::ControlGlance(CrossingPhase phase) {
    AOIProbabilities scaledAOIProbs = LookUpControlAOIProbability(phase);
    auto aoi = static_cast<ControlAOI>(sampler.SampleIntProbability(scaledAOIProbs));

    if (worldRepresentation.egoAgent->GetDistanceOnIntersection() > 0 && (phase > CrossingPhase::Deceleration_TWO)) {
        // control gazes on intersection
        return ControlGlanceOnXIntersection(aoi, phase);
    } else if (worldRepresentation.egoAgent->GetDistanceToNextIntersection() > 0 && phase < CrossingPhase::Deceleration_TWO) {
        // control gazes in front of intersection
        return ControlGlanceOnRoad(aoi);
    } else {
        // control gazes while approaching an intersection
...
GazeState XIntersection::ControlGlanceOnRoad(ControlAOI aoi) {
    GazeState gazeState;
...
const Common::Vector2d* XIntersection::FixationPointForCGOnRoad(const std::vector<Common::Vector2d>& fixPoints, ControlAOI aoi) {
...
GazeState XIntersection::ControlGlanceOnXIntersection(ControlAOI aoi, CrossingPhase phase) {
    GazeState gazeState;

    auto fixationPoint = FixationPointForCGOnXIntersection(controlFixPointsOnXIntersection, phase, aoi);
    gazeState.fixationState = {GazeType::ControlGlance, static_cast<int>(aoi)};
    gazeState.target.fixationPoint = *fixationPoint;
    gazeState.openingAngle = 100 * (M_PI / 180); // TODO replace by realistic behaviour
    gazeState.fixationDuration = 800;            // TODO realistic distribution
...
const Common::Vector2d* XIntersection::FixationPointForCGOnXIntersection(const std::vector<Common::Vector2d>& fixPoints,
                                                                         CrossingPhase phase, ControlAOI aoi) {
    const Common::Vector2d* fixPoint;
    switch (phase) {
    ...
    case CrossingPhase::Crossing_Left_TWO:
        ...
        break;
    default:
        std::string message = __FILE__ " Line: " + std::to_string(__LINE__) + "Control fixation point is not defined: useful field of view angle can not be calculated!";
        throw std::runtime_error(message);
    }
    return fixPoint;
}

double XIntersection::LookUpFixationDuration(CrossingPhase phase, ScanAOI aoi) {}

AOIProbabilities XIntersection::LookUpControlAOIProbability(CrossingPhase phase) {
    ...
    if(importedProbs.find(phase) != importedProbs.end()) {
        for(auto& prob : importedProbs.at(phase)) {
            aoiProbs.insert(std::make_pair(static_cast<int>(prob.first), prob.second));
        }
    } else {
...
//        aoiProbs.insert(std::make_pair(static_cast<int>(ControlAOI::Left), 0.20));
//        aoiProbs.insert(std::make_pair(static_cast<int>(ControlAOI::Oncoming), 0.3));
//        aoiProbs.insert(std::make_pair(static_cast<int>(ControlAOI::Right), 0.5));
...
//        aoiProbs.insert(std::make_pair(static_cast<int>(ControlAOI::Left), 0.20));
//        aoiProbs.insert(std::make_pair(static_cast<int>(ControlAOI::Oncoming), 0.4));
//        aoiProbs.insert(std::make_pair(static_cast<int>(ControlAOI::Right), 0.4));

//    } else if (phase == CrossingPhase::Crossing_Right) {
//        // turning right do not have oncoming viewing point
//        aoiProbs.insert(std::make_pair(static_cast<int>(ControlAOI::Left), 0.40));
//        aoiProbs.insert(std::make_pair(static_cast<int>(ControlAOI::Right), 0.6));

//    } else if (phase == CrossingPhase::Crossing_Left_ONE) {
//        aoiProbs.insert(std::make_pair(static_cast<int>(ControlAOI::Left), 0.20));
//        aoiProbs.insert(std::make_pair(static_cast<int>(ControlAOI::Oncoming), 0.5));
//        aoiProbs.insert(std::make_pair(static_cast<int>(ControlAOI::Right), 0.3));
...
AOIProbabilities XIntersection::LookUpScanAOIProbability(CrossingPhase phase) {
    ...
    //(use values of "give priority" for unregulated intersection (priority to the right)
    if (worldRepresentation.egoAgent->GetIndicatorState() == IndicatorState::IndicatorState_Left) {
        if (worldRepresentation.agentMemory->size() == 0) {
            ...
            case CrossingPhase::Approach:
                ...
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Straight),
                                               stochastics->GetNormalDistributed(0.754022988505747, 0.260077969987969)));
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Left),
                                               stochastics->GetNormalDistributed(0.122742200328407, 0.232515910096245)));
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Right),
                                               stochastics->GetNormalDistributed(0.0864532019704433, 0.13733384909178)));
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Other),
            ...
            case CrossingPhase::Crossing_Left_TWO:
                [[fallthrough]];
            case CrossingPhase::Crossing_Straight:
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Dashboard), 0));
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Straight),
                ...
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Right), 0));
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Other),
            ...
            case CrossingPhase::Exit:
                ...
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Left), 0));
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Right),
            ...
            case CrossingPhase::Approach:
                ...
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Other), 0));
                break;
            case CrossingPhase::Deceleration_ONE:
                aoiProbs.insert(std::make_pair(static_cast<int>(ScanAOI::Dashboard),
