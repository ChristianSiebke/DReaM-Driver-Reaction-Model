...
public:
    ...
    void EndTimePoint(std::string identifier = "") {
        ...
        auto time = std::chrono::duration_cast<std::chrono::milliseconds>(endTimePoint - openPoints[actualIdentifier]);

        if (measuredTimes.find(actualIdentifier) == measuredTimes.end()) {
            std::vector<std::chrono::milliseconds> tmp;
...
private:
    double CalculateMean(std::vector<std::chrono::milliseconds> values) {
        double total = 0;
    ...
    double CalculateMedian(std::vector<std::chrono::milliseconds> values) {
        std::sort(values.begin(), values.end());
        if (values.size() % 2 != 0)
            return (double)values[values.size() / 2].count();
        return (double)(values[(values.size() - 1) / 2].count() + values[values.size() / 2].count()) / 2.0;
    }

private:
    std::string name;
    std::unordered_map<std::string, std::vector<std::chrono::milliseconds>> measuredTimes;
